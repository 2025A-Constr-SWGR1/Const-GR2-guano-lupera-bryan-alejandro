...
public class CalificacionDAO {
    ...
    public double calcularPromedioAtributo(Long idRestaurante, String atributo) {
        EntityManager em = null;
        try {
            em = emf.createEntityManager();
            String jpql = "SELECT AVG(c." + atributo + ") FROM Calificacion c WHERE c.restaurante.id = :idRestaurante";
            TypedQuery<Double> query = em.createQuery(jpql, Double.class);
            query.setParameter("idRestaurante", idRestaurante);
            Double promedio = query.getSingleResult();
            return promedio != null ? promedio : 0.0;
        } catch (Exception e) {
            LOGGER.log(Level.WARNING, "Error al calcular promedio de " + atributo, e);
            return 0.0;
        } finally {
            if (em != null && em.isOpen()) {
                em.close();
