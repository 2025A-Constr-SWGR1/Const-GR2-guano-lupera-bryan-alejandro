...
public class PlanificacionService {
    ...
    public Planificacion crearPlanificacion(String nombre, String hora, Integer limiteComensales, Comensal comensal) {
        validarParametrosCreacion(nombre, hora);
        if (limiteComensales != null && limiteComensales < 1) {
            throw new IllegalArgumentException("El límite de comensales debe ser mayor a 0");
        }
        Planificacion planificacion = new Planificacion(nombre, hora, limiteComensales);
        if (comensal == null) {
            throw new IllegalArgumentException("Comensal no encontrado");
        }
        planificacion.setComensalPlanificador(comensal);
    ...
    public void confirmarRestauranteDelGrupo(Planificacion planificacion, String restauranteConfirmado) {
        ...
        for (Comensal comensal : comensales) {
            // notificacionService.notificarRestauranteElegido(comensal, "Se ha confirmado:
            // " + restauranteConfirmado);
        }
    }
    ...
    public void iniciarVotacion(Long planificacionId) {
        ...
        // "Se ha iniciado la votación para la planificación: " +
