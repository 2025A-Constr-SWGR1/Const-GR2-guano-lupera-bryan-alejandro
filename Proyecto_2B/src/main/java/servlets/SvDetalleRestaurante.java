...
// Refactorización del método doGet para mejorar la legibilidad y manejo de errores (extract method)
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            // Verificar si el usuario ya votó por el menú del día
            HttpSession session = req.getSession(false);
            if (session != null && session.getAttribute("usuario") != null && restaurante.getMenuDelDia() != null) {
                Usuario usuario = (Usuario) session.getAttribute("usuario");

                // Crear instancia del DAO para verificar votos
                EntityManagerFactory emfVoto = Persistence.createEntityManagerFactory("UFood_PU");
                VotoMenuDelDiaDAO votoDAO = new VotoMenuDelDiaDAO(emfVoto);

                boolean yaVoto = votoDAO.buscarVotoPorUsuarioYRestaurante(usuario.getId(), restaurante.getId()) != null;
                req.setAttribute("usuarioYaVoto_" + restaurante.getId(), yaVoto);
