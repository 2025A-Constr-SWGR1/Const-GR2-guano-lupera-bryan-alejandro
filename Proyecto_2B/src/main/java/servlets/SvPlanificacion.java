...
public class SvPlanificacion extends HttpServlet {
    ...
    private void procesarPostCrearPlanificacion(HttpServletRequest request, HttpServletResponse response)
        ...
        String limiteComensalesStr = request.getParameter("limiteComensales");
        Long idComensalPlanificador = obtenerIdComensalPlanificador(request);
        String mensaje;
        ...
        try {
            ...
            // Verificar si se proporcionó límite de comensales
            if (limiteComensalesStr != null && !limiteComensalesStr.trim().isEmpty()) {
                Integer limiteComensales = Integer.parseInt(limiteComensalesStr);
                planificacion = planificacionService.crearPlanificacion(nombre, hora, limiteComensales, comensal);
            } else {
                planificacion = planificacionService.crearPlanificacion(nombre, hora, comensal);
            }
            ...
            mensaje = "Error: El límite de comensales debe ser un número válido.";
        } catch (Exception e) {
    ...
    private void mostrarMisPlanificaciones(HttpServletRequest request, HttpServletResponse response)
        ...
        Long idComensal = obtenerIdComensalPlanificador(request); // Reutilizamos este método que extrae el ID del
                                                                  // comensal
